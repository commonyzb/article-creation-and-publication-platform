		...
		setPager = function (pgid, tp){
			...
			if(ts.p.pginput===true) {
				$("#"+pgcnt).on('keypress','input.ui-pg-input', function(e) {
					...
					if(key === 13) {
						if(!clearVals('userPage', this)) { return false; }
...
$.jgrid.extend({
	editCell : function (iRow,iCol, ed, event){
		return this.each(function (){
			...
			iCol = parseInt(iCol,10);
			// select the row that can be used for interceptor methods
	...
	return this.each( function() {
		...
		*/
		...
		this.createTableRowForRule = function(rule, group ) {
			...
			// populate drop down with userPage provided column definitions
			var j=0;
			for (i = 0; i < that.p.columns.length; i++) {
				// but show only serchable and serchhidden = true fields
				var searchable = (that.p.columns[i].search === undefined) ? true: that.p.columns[i].search,
				...
				if(!newObj) {
					...
					if(ylen>=1) {
						...
						for(k=0;k<ylen;k++) {
							...
							// Check to see if we have userPage defined conditions
